...
<%@page import="bean.*"%>
<jsp:useBean id="pages" scope="page" class="bean.SpiltPageRank"/>
...
<%
    ...
    sqlStr="select *"
            + "from ("
            + "         select (@rank := @rank + 1) no, temp.*"
            + "         from (select t2.user_name, sum(t1.SUB_SCORE)"
            + "               from score t1"
            + "                        inner join student t2 on t1.USER_ID = t2.user_id"
            + "               group by t1.USER_ID"
            + "               order by sum(t1.SUB_SCORE)"
            + "              ) temp,"
            + "              (SELECT @rank := 0) b) t";
    //查询数据表，获得查询结果
    String strPage = null;
